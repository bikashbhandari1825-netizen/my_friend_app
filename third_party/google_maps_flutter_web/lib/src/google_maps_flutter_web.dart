...
class GoogleMapsPlugin extends GoogleMapsFlutterPlatform {
  ...
  ///
  /// PATCHED (KaamMitra, see third_party/google_maps_flutter_web/PATCH.md):
  /// upstream unconditionally routed this through `_map(mapId)`, which
  /// `assert`s that a controller was already registered — but a `GoogleMap`
  /// widget can be disposed before Flutter ever calls
  /// `buildViewWithConfiguration` for it (e.g. it never got a chance to
  /// paint, such as being torn down within the same frame it was created).
  /// That is a legitimate, harmless case — disposing something that was
  /// never fully built is a no-op, not an error — so we look the controller
  /// up directly here instead of asserting on it.
  @override
